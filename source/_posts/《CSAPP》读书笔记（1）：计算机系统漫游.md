...
汇编的过程包括词法分析、语义分析、中间代码优化等细节，汇编语言相对C语言这类高级语言也更加低级且与计算机的底层硬件密切相关。本书不会过多的展开，对这部分感兴趣的可以在读完本书后去学习**编译原理**

* **汇编**：将汇编程序翻译成**机器语言指令**，并将其打包成一种叫可重定位目标程序(relocatable object program)的二进制文件
* **链接**：链接器将各个.o文件合并成可执行文件。链接器使得分离编译成为可能。在编写大型程序时，通常会使用到各种函数库，但是我们代码中并没有这些函数的具体实现，所以就需要在链接阶段将该函数的具体实现合并，最终得到可执行目标文件。
